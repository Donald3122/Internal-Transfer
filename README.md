Сделал переводы между считами, при переводе с считов с разной валютой происходит конвертация и коректно сожаются суммы так же добавлен Check запрос для получения всех считов клиента, и затем можем использовать эти данные для самого перевода (Pay) 
Максимально старался соблюдать SOLID и делать код чистым и понятным для того кто будет его читать и понимать)
